...
def infer(weights, source, img_size, conf_thres, iou_thres, classes, name, save_dir,Output_Name,Base64_Encoded_Image=None):
    ...
    CMD=f"python3 infer.py --weights {weights} --source {source} --img-size {img_size} --conf-thres {conf_thres} --iou-thres {iou_thres} --classes {classes} --name {name} --save-txt --save-dir {save_dir}"
    os.system(CMD)
